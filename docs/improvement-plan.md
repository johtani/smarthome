...
- [ ] `subcommand/subcommand.go:34` - ignoreError時のエラーメッセージ処理を決定・実装
- [ ] `subcommand/subcommand.go:77,184` - Did You Mean機能の候補ソート実装
...
- [ ] `fmt.Printf`を構造化ログ（slog）に置き換え
- [ ] ログレベル（Debug/Info/Error）の適切な使い分け
- [ ] `server/slack/handler.go` の "######### :" プレフィックスを適切なログに変更

## フェーズ2: パフォーマンスと安全性

### 4. キャッシュ実装の最適化

- [ ] `subcommand/action/switchbot/client.go` - キャッシュの不要なリセットを削除
- [ ] 並行アクセス対策（sync.RWMutex）の追加検討

### 5. HTTPクライアント処理の共通化

- [ ] レスポンス処理の重複コード削減
- [ ] 共通ヘルパー関数の作成（defer/Close/StatusCodeチェック）
