...
| `SMARTHOME_RESOLVER_MODE` | `config.json` の `resolver.mode` (`legacy`/`dspy`) |
| `SMARTHOME_RESOLVER_FEEDBACK_ENABLED` | `config.json` の `resolver.feedback_enabled` |
| `SMARTHOME_RESOLVER_PROMPT_VERSION` | `config.json` の `resolver.prompt_version` |
| `SMARTHOME_RESOLVER_DSPY_ENDPOINT` | `config.json` の `resolver.dspy_endpoint` |
| `SMARTHOME_RESOLVER_DSPY_TIMEOUT_SECONDS` | `config.json` の `resolver.dspy_timeout_seconds` |
...
`request_id` と `trace_id` を軸に、定期バッチで抽出・結合する想定です。

#### OTel Collector サンプルパイプライン

`config/otel-collector-resolver.sample.yaml` をベースに、トレースを通常のOTLP先に送るのに加えて、同時にファイルへも保存できます。

```yaml
exporters:
  otlphttp:
    endpoint: http://localhost:4318
  file/resolver_traces:
    path: ./tmp/otel/resolver-traces.jsonl
```

このファイル出力を、定期ジョブでCSV化して学習データ作成の前処理に使います。

#### 抽出ジョブ（PowerShell）

Collectorの `file` exporter 出力から `resolver.decision / resolver.execution / resolver.feedback` を抽出するスクリプト:

`tools/resolver-events/extract-from-collector-file.ps1`

実行例:

```powershell
pwsh tools/resolver-events/extract-from-collector-file.ps1 `
  -InputPath .\tmp\otel\resolver-traces.jsonl `
...
`search_after` によりページングし、出力先には次の4ファイルを生成します。
...
1. ResolverイベントCSVからデータセット生成
2. DSPyで最適化
3. オフライン評価ゲート（command/args精度しきい値）

詳細は [tools/dspy/README.md](./tools/dspy/README.md) を参照してください。
