...
### 開発ガイドライン
- **Issueの扱い**:
  - 「Issue 108」のように番号でIssueが指定された場合、GitHubのIssueとして扱い、内容を確認（検索やリポジトリ内のドキュメント参照）するようにしてください。
- **ブランチ作成**:
...
- **ドキュメント**:
  - 新しい機能の実装や設定の追加を行った場合は、`README.md` にその内容を追記してください。

### 完了・コミット時のルール
- **コード品質**: コミット前に必ず `go fmt ./...` と `golangci-lint run ./...` を実行し、コードを整形し、静的解析エラーがないことを確認してください。
- **動作確認**: 変更後は必ず `go test ./...` を実行し、テストがパスすることを確認してください。
- **手順**:
  1. `go fmt ./...`
  2. `golangci-lint run ./...`
  3. `go test ./...`
  4. すべてパスした場合のみコミットを作成する。
