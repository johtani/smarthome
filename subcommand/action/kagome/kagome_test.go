...
func TestKagomeAction_Run(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ipa -search -filter", fields: fields{name: "ipa", dictionary: IPA}, args: args{args: "-search -filter ipaだよ"}, want: "```\nipa\t名詞,一般,*,*,*,*,*\n```", wantErr: false},
		{name: "ipa -search -filter empty strings", fields: fields{name: "ipa", dictionary: IPA}, args: args{args: "-search -filter"}, want: "```\n```", wantErr: false},
	}
